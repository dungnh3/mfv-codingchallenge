...
init-db-test: create-db waiting-start-container
	docker exec -it database mysql -u root -psecret -e "CREATE DATABASE mfv_test;" || true
	migrate -source "file://migrations" -database "mysql://$(USER):$(PASSWORD)@tcp($(HOST):$(PORT))/mfv_test" up
.PHONY: init-db-test
